...
## Ограничения данных в витрине

| Тип данных                                 | Допуск к витрине | Обоснование                                                              |
| ------------------------------------------ | ---------------- | ------------------------------------------------------------------------ |
| **Медицинские карты**                      | ❌ Исключаются    | Содержат чувствительную информацию, не подлежащую аналитике              |
| **Истории болезней**                       | ❌ Исключаются    | Индивидуальная медицинская тайна, не требуется для бизнес-аналитики      |
| **Результаты исследований**                | ❌ Исключаются    | Могут содержать идентифицирующие признаки и не используются в отчётности |
| **Агрегированные/обезличенные показатели** | ✅ Допускаются    | Необходимы для анализа эффективности, не нарушают конфиденциальность     |
| **Финансовые данные по услугам**           | ✅ Допускаются    | Используются для финансового анализа, при условии обезличивания          |

### Меры обеспечения ограничений

Для гарантированного исключения недопустимых данных из витрины предлагается внедрить следующие технические и организационные меры:

1. **Фильтрация и анонимизация на этапе ETL (Batch Processing)**
	- **Компонент:** `Batch processing [ELT: Apache Airflow]`
	- **Действие:** В DAG-ах Airflow добавлены специализированные операторы, которые:
	  - Исключают поля с персональными данными (ФИО, адрес, СНИЛС, номер полиса) на этапе экстракции.
	  - Заменяют идентификаторы пациентов на суррогатные (псевдонимизация), если необходимо отслеживать динамику, но без возможности деанонимизации.
	  - Не загружают текстовые поля, содержащие описания историй болезней и результаты исследований.
	  - Применяют агрегацию (например, группировку по полу, возрасту, отделению) для статистических отчётов.

2. **Слой анонимизации медицинских данных**
	- **Компонент** на базе Amazon Redshift Dynamic Data Masking
	- **Действие:** загружается только из `Medical Data` через процесс анонимизации, исключая запрещённые категории.

3. **Доменная ответственность в Data Mesh**
	- **Компонент:** `Data Mesh`
	- **Действие:** Каждый домен (например, «Медицинские услуги», «Финансы») сам отвечает за подготовку своих данных перед публикацией в общую витрину. В контрактах данных явно прописывается, какие поля исключаются и какие агрегации применяются. Это гарантирует, что в общую шину и витрину попадают только разрешённые данные.

4. **Мониторинг и аудит**
	- **Компонент:** Внедрена служба централизованного аудита
	- **Функция:** Логирование всех запросов к витрине и процессов загрузки. При обнаружении попыток доступа к недопустимым данным срабатывает алерт.
